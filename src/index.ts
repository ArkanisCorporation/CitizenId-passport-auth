...
export class Strategy extends OAuth2Strategy {
    ...
    private _normalizeProfile(json: CitizenIDUserInfo, raw: string): CitizenIDProfile {
        ...
        if (json[GoogleClaimKeys.AVATAR_URL]
            || json[GoogleClaimKeys.EMAIL]
            || json[GoogleClaimKeys.NAME]
            || json[GoogleClaimKeys.ACCOUNT_ID]) {
            profile.google = {
        ...
        if (json[TwitchClaimKeys.AVATAR_URL]
            || json[TwitchClaimKeys.EMAIL]
            || json[TwitchClaimKeys.USERNAME]
            || json[TwitchClaimKeys.ACCOUNT_ID]) {
            profile.twitch = {
        ...
        if (json[DiscordClaimKeys.AVATAR_URL]
            || json[DiscordClaimKeys.USERNAME]
            || json[DiscordClaimKeys.ACCOUNT_ID]
            || json[DiscordClaimKeys.SCOPES]) {
            profile.discord = {
        ...
        if (json[RSIClaimKeys.AVATAR_URL]
            || json[RSIClaimKeys.USERNAME]
            || json[RSIClaimKeys.ENLISTED_AT]
            || json[RSIClaimKeys.CITIZEN_ID]
    ...
    getOAuthAccessToken(code: string, params: Record<string, unknown>, callback: (err: Error | null, accessToken?: string, refreshToken?: string, params?: OAuth2TokenParams) => void): void {
        ...
        originalGetOAuthAccessToken.call(this, code, params, (err: Error | null, accessToken?: string, refreshToken?: string, _params?: OAuth2TokenParams) => {
            if (err) {
                return callback(err);
            }

            // Store the ID token for profile extraction
            if (_params?.id_token && typeof _params.id_token === 'string') {
                this._idToken = _params.id_token;
            }

            callback(null, accessToken, refreshToken, _params);
        });
    }
